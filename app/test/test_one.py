...
async def test_two_post_auth1(clean_client):
    token_response = await clean_client.post('/auth/token', data={
        'username': 'frensski',
        'password': '123gmail'
    })
...
async def test_four_post_tasks(clean_client):
    login_response = await clean_client.post('/auth/token', data={
        'username': 'frensski',
        'password': '123gmail'
    })
    ...
    response = await clean_client.post('/tasks/', json=json_data, headers=headers)

    assert response.status_code == 403
    assert response.json()['detail'] == 'This name already exists or the id was not found or invalid'
...
async def test_seven_post_tasks(clean_client):
    ...
    assert response.status_code == 403
    assert response.json()['detail'] == 'This ID is already taken by another user or does not exist'
...
async def test_eight_put_tasks(clean_client):
    ...
    response = await clean_client.put('/tasks/', params={'task_id': 9}, json=json_data, headers=headers)
...
async def test_twelve_delete_tasks(clean_client):
    ...
    response = await clean_client.delete('/tasks/', params={'task_id': 9}, headers=headers)
...
async def test_fiveteen_put_users(clean_client):
    ...
    response = await clean_client.put('/users/', params={'user_id': 9}, json=tdata, headers=headers)
...
async def test_eighteen_put_users(clean_client):
    ...
    response = await clean_client.put('/users/', params={'user_id': 33}, json=tdata, headers=headers)
